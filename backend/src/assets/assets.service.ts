...
export class AssetsService {
  ...
  async dissociate(assetId: string) {
    const asset = await this.repo.findOne(assetId);

    console.log(asset)
    if (!asset) {
      throw new NotFoundException(`Asset com ID ${assetId} não encontrado`);
    }

    if (!asset.employeeId) {
      throw new BadRequestException(
        'Este asset já está desvinculado de qualquer funcionário.',
      );
    }

    const updatedAsset = await this.repo.update(assetId, {
      employeeId: null,
      status: 'DISPONIVEL',
    });
    ...
    return {
      ...
      data:plainToInstance(ResponseAssetDto, updatedAsset, {
      excludeExtraneousValues: true,
    })
  };
  }
  ...
  async update(
    ...
    return plainToInstance(
      ResponseAssetDto,
      this.repo.update(id, updateAssetDto),
    );
  }
